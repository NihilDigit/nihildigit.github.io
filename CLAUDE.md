...
- **Inline links in prose** (Markdown notes via `app-prose`, hand-built body prose like `about.astro` / hero tagline): always-on **dotted** underline (`decoration-dotted underline-offset-4`), text color stays inherited, hover flips text to accent. Underline-as-affordance, not hover-revealed; the prose density on this site is low enough that "dotted soup" doesn't actually trigger.
- **Archive / CTA affordance links** (note titles in `Card.astro`, the 404 "Go back home" link): **dashed** underline (`decoration-dashed`). Visible always for CTAs; hover-only for archive list items where the link sits next to its own metadata.
- **Eyebrow + content-title pattern** is canonical for sectioned long pages (`/anvil`, `/papers`, `/projects`, `/about`). The eyebrow is `<h2 class="font-mono text-xs uppercase tracking-widest text-muted-foreground">` (small mono label, e.g. `TL;DR`, `Architecture`, `Publications`); when a section has a content title underneath, that title is `<h3 class="text-2xl font-semibold tracking-tight">`. Visually the h2 is smaller than its h3 — that is the editorial pattern, not a bug. Don't "fix" it by reversing the levels: the AOM tree is correct (h1 → h2 section → h3 sub-content), and the type contrast (eyebrow as orienting marker, content title as focus) is the design point. Sections with no separate content title (figures, tables, lists, single-paragraph blocks) just use the eyebrow `<h2>` as the section heading.
- **Foreground tiers — three only**: `text-foreground` for default body/headings, `text-foreground/80` for lead/secondary copy (TL;DRs, card descriptions, table caption columns, abstracts), `text-muted-foreground` for metadata/captions/eyebrow labels. Never invent `/85` / `/90` / `/70` between these — if a fourth shade feels needed, the layout is doing too much.
